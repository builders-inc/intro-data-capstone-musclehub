...
app_pivot = df.groupby(['ab_test_group','is_application']).first_name.count().reset_index().pivot(columns='is_application',index='ab_test_group',values='first_name')
print app_pivot
...
pval = binom_test(325, n=2500, p=0.0998)
print pval


# pval from binomial test is well below 0.05 showing that there is a significant difference between applications from A and applications from B
...
from scipy.stats import binom_test
pval_app = binom_test(250, n=325, p=.8)
print pval_app
...
from scipy.stats import binom_test
pval_total = binom_test(250, n=2500, p=.0798)
print pval_total
...
plt.savefig('vistors_who_purchase.png')
plt.show()
